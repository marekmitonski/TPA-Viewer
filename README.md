...
### Generator szafek
Zakładka „Generator szafek" składa listę formatek z definicji szafek (wzory wymiarów z parametrami `H/W/D/T` + własnymi) i zapisuje pliki TCN o kolejnych numerach (hex/dec) do wskazanego katalogu, z etykietami i kodami kreskowymi:
- **Projekty** – praca odbywa się w projektach (np. per klient): u góry lista projektów (z edycją nazwy i usuwaniem) i przycisk „＋ Nowy projekt"; w projekcie dodaje się kolejne szafki, każda z typem wybranym z listy, grafiką poglądową i własnym kompletem parametrów; „Generuj listę formatek" buduje wspólną listę elementów wszystkich szafek projektu
- **Kody szafek** – każdy projekt dostaje losowy, stały 2-literowy prefiks; szafki w projekcie otrzymują kody prefiks+numer (np. `KT1`, `KT2` – numer nie zmienia się po usunięciu innej szafki), widoczne w kolumnie „Kod szafki" listy formatek i na etykietach
- **Konstruktor szafek** – edycja definicji szafek (dawne „Edytuj szafki") wymaga PIN-u; PIN przechowywany w `ustawienia.json` → `opts.pin` (domyślnie `0000`), pytany raz na sesję, aplikacja zawsze startuje w trybie „Generuj"
- **Trwała lista formatek** – lista formatek należy do projektu (wyświetla się pod jego szafkami) i jest zapisywana razem z nim; po powrocie do zakładki otwiera się ostatnio używany projekt z szafkami i formatkami. Edycja wymiaru formatki, która ma już plik, oznacza ją „↻" – ponowne „Generuj pliki" **nadpisuje ten sam plik** (kod/nazwa bez zmian, nowa zawartość); nowe kody dostają tylko wiersze bez pliku. „Generuj formatki i pliki TCN" przy istniejących plikach pyta o potwierdzenie (buduje listę od nowa)
- **Jedno kliknięcie** – „Generuj formatki i pliki TCN" buduje listę i od razu zapisuje pliki z kodami. Kody plików mają stałą długość 10 znaków, dopełniane zerami i numerowane od zera (`0000000000.TCN`, `0000000001.TCN`, …); w niepustym katalogu numeracja kontynuuje od największego znalezionego numeru
- **Grafika szafki** – do definicji szafki można wgrać obrazek (przeskalowywany do miniatury), widoczny przy szafce w projekcie
- **Element bez szablonu TCN nie dostaje pliku ani kodu kreskowego** – trafia tylko na listę formatek (do cięcia)
